#! /bin/bash
if [ 16 -ne $(node --version | cut -c 2- | cut -d '.' -f 1) ]; then echo "Project currently requires Node 16.x.x" >&2; exit 1; fi
mkdir -p dist && cp -v _redirects dist/ && cp -v _headers dist/ && cp -Rv src/assets/* dist/ && cp -Rv src/meta/* dist/
...
cd src/react && npm run build && cd ../..
cp -R src/react/build/* dist/projects/